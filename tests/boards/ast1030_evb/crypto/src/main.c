...
static const struct device *const hace_dev = DEVICE_DT_GET(DT_NODELABEL(hace));
...
static int _sha_test(const struct hash_testvec *tv, int tv_len, enum hash_algo algo)
{
	...
	for (i = 0; i < tv_len; i++) {
		...
		ini.flags = CAP_SYNC_OPS | CAP_SEPARATE_IO_BUFS;
		...
		/*
		 * hash_compute() alone does NOT feed pkt->in_buf to the
		 * aspeed HACE driver - aspeed_hash_compute(finish=true)
		 * calls aspeed_hash_final() directly, which only finalizes
		 * whatever aspeed_hash_update() already staged into the
		 * driver's internal buffer (see drivers/crypto/hace_aspeed.c).
		 * Skipping hash_update() silently hashes empty input every
		 * time. Two calls, matching the old hash_update()+hash_final()
...
{
	const struct device *dev = DEVICE_DT_GET(DT_NODELABEL(rsa));
	struct rsa_ctx ini;
	struct rsa_pkt pkt;
	struct rsa_key *rk;
	char plain_text[512];
...
{
	ast_zassert_true(device_is_ready(hace_dev), "HACE device is not ready");
	ast_zassert_true(device_is_ready(DEVICE_DT_GET(DT_NODELABEL(rsa))),
			  "RSA device is not ready");

	ast_zassert_equal(sha256_test(), 0, "sha256 test failed");
	ast_zassert_equal(sha384_test(), 0, "sha384 test failed");
	ast_zassert_equal(sha512_test(), 0, "sha512 test failed");
...
ZTEST(crypto, test_crypto_all)
{
	zassert_equal(test_crypto(), AST_TEST_PASS, "crypto test failed");
}
