...
{
	int ret;
	const struct device *gpio_dev = device_get_binding(BMC_RSTIND_GPIO_DEV_NAME);
	...
	ret = gpio_pin_interrupt_configure(gpio_dev, BMC_RSTIND_GPIO_PIN_IN,
					   GPIO_INT_DISABLE);
	if (ret != 0) {
	...
	ret = gpio_pin_configure(gpio_dev, BMC_RSTIND_GPIO_PIN_IN, GPIO_INPUT);
	if (ret != 0) {
		printk("demo_err: cannot configure gpio device.\n");
		return;
	}

	gpio_init_callback(&gpio_cb, gpioo3_callback, BIT(BMC_RSTIND_GPIO_PIN_IN));
