...
static void peci_aspeed_isr(const struct device *dev)
{
	...
	struct peci_register_t *peci_register = DEV_CFG(dev)->base;
...
static int peci_aspeed_init(const struct device *dev)
{
	LOG_DBG("%s", __func__);
	struct peci_register_t *peci_register = DEV_CFG(dev)->base;
	union control_register_t control;
	...
	reset_line_assert_dt(&DEV_CFG(dev)->reset);
...
{
	struct peci_register_t *peci_register = DEV_CFG(dev)->base;
	union timing_negotiation_register_t timing_negotiation;
	union control_register_t control;
...
static int peci_aspeed_transfer(const struct device *dev, struct peci_msg *msg)
{
	struct peci_aspeed_data *peci_data = DEV_DATA(dev);
	struct peci_register_t *peci_register = DEV_CFG(dev)->base;
	union read_write_length_register_t peci_header;
	uint32_t tx_len, rx_len;
	uint32_t reg_index, byte_index;
	union write_data_register_t write_val;
	uint32_t status;
	int ret = 0, flag_ret;
	...
	LOG_DBG("msg header addr= 0x%08x, tx_len = 0x%08x, rx_len = 0x%08x , cmd_code = 0x%08x, "
		"flag = 0x%08x\n",
		msg->addr, msg->tx_buffer.len, msg->rx_buffer.len, msg->cmd_code, msg->flags);
	/* set peci header */
	...
	if (flag_ret != BIT(PECI_INT_CMD_DONE)) {
		if (flag_ret < 0) {
			if (flag_ret == osFlagsErrorTimeout) {
				...
				peci_aspeed_configure(dev, DEV_DATA(dev)->freq / 1000);
...
#define ASPEED_PECI_DEVICE_INIT(inst)                                                              \
	static const struct peci_aspeed_config peci_aspeed_cfg_##inst = {                          \
		.base = (struct peci_register_t *)DT_INST_REG_ADDR(inst),                          \
	...
	static struct peci_aspeed_data peci_aspeed_data_##inst;                                    \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, peci_aspeed_init, NULL, &peci_aspeed_data_##inst,              \
			      &peci_aspeed_cfg_##inst, POST_KERNEL, CONFIG_PECI_INIT_PRIORITY,     \
			      &peci_aspeed_driver);
