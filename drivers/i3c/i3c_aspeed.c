...
static int aspeed_i3c_do_daa(const struct device *dev)
{
	...
	do {
		...
		if (ret) {
			...
			if (ret == RESPONSE_ERROR_IBA_NACK || ret == -ETIMEDOUT) {
				break;
			}
...
static void aspeed_i3c_target_rst_worker(struct k_work *work)
{
	struct aspeed_i3c_data *data =
		CONTAINER_OF(work, struct aspeed_i3c_data, target_rst_work);
	const struct i3c_target_callbacks *cbs = data->target_config->callbacks;

	aspeed_i3c_init(data->dev);
	if (cbs && cbs->rst_cb)
		cbs->rst_cb(data->target_config);
}
						 ...
						 struct i3c_ibi *notifier)
{
	...
	if (k_sem_take(&data->target_ibi_sem, K_MSEC(1000))) {
		...
		k_work_submit(&data->target_rst_work);
		return -EIO;
	}
