...
static void otp_dev_init(const struct shell *shell)
{
	if (!dev)
		dev = device_get_binding(OTP_DRV_NAME);

	if (!dev)
		shell_printf(shell, "device get binding %s failed\n", OTP_DRV_NAME);
}
...
static void otp_read_conf(uint32_t offset, uint32_t *data)
{
	...
	otp_read(dev, config_offset, data);
}
...
static void _otp_prog_bit(uint32_t value, uint32_t prog_address, uint32_t bit_offset)
{
	...
	otp_program(dev, prog_address, prog_bit);
}
...
static int otp_print_conf(const struct shell *shell, uint32_t offset, int dw_count)
{
	...
	err = otp_set_soak(dev, 0);
	if (err) {
		shell_printf(shell, "otp set soak failed\n");
		return err;
	}
	...
	for (i = start; i < start + count; i++) {
		...
		if (strap_status[i].protected == 1) {
			...
			if (strap_status[i].remain_times == 0)
				...
				shell_printf(shell, "and still can write %d times",
					     strap_status[i].remain_times);
		}
		shell_printf(shell, "\n");
	}
...
static int otp_print_scu_image(const struct shell *shell, struct otp_image_layout *image_layout)
{
	...
	for (i = 0; i < info_cb.scu_info_len; i++) {
		...
		if (scu_info[i].length == 1)
			...
			shell_printf(shell, "0x%-2X:0x%-6x", bit_offset,
					bit_offset + scu_info[i].length - 1);
	...
	for (i = 0; i < info_cb.strap_info_len; i++) {
		...
		if (view) {
			for (j = 0; j < length; j++) {
				...
				shell_printf(shell, "%-9d",
					strap_status[bit_offset + j].remain_times);
				shell_printf(shell, "0x%-7X",
					strap_status[bit_offset + j].protected);
				if (strap_info[i].value == OTP_REG_RESERVED) {
					shell_printf(shell, " Reserved\n");
					continue;
				}
			 ...
			 struct otp_image_layout *image_layout, uint32_t *data)
{
	...
	shell_printf(shell, "Start Programing Data...\n");
	...
	shell_printf(shell, "Program ECC region done\n");
	...
	shell_printf(shell, "Program Data region done\n");
			 ...
			 uint32_t *otp_conf)
{
	...
	shell_printf(shell, "Start Programing Conf...\n");
	...
	for (i = 0; i < 16; i++) {
		...
		pass = 0;
		for (k = 0; k < RETRY; k++) {
			if (verify_dw(prog_address, &conf[i], &conf_ignore[i],
				      compare, 1) != 0) {
				otp_set_soak(dev, 2);
		...
		if (pass == 0) {
			shell_printf(shell, "address: %08x, otp_conf: %08x, input_conf: %08x, "
					"mask: %08x\n",
				...
				uint32_t *scu_pro)
{
	...
	shell_printf(shell, "Start Programing SCU protect...\n");
	...
	for (i = 0; i < 2; i++) {
		...
		pass = 0;
		for (k = 0; k < RETRY; k++) {
			if (verify_dw(prog_address, &OTPSCU[i], &OTPSCU_IGNORE[i],
				      compare, 1) != 0) {
				otp_set_soak(dev, 2);
	...
	for (i = 0; i < data_dw - 2; i++) {
		...
		if (i % 2 == 0) {
			if ((data_masked | buf_masked) == buf_masked) {
				...
				shell_printf(shell, "Input image can't program into OTP, "
					"please check.\n");
...
static int otp_prog_image(const struct shell *shell, int addr, int nconfirm)
{
	...
	if (image_soc_ver != info_cb.version) {
		shell_printf(shell, "Image SOC version (0x%x) is not match to "
			"HW SOC version (0x%x)\n",
	...
	case 1:
		shell_printf(shell, "WARNING: OTP image is not generated by otptool v2.x.x\n");
		shell_printf(shell, "Please use the latest version of otptool to "
			"generate OTP image\n");
	...
	case 2:
		...
		break;
	default:
		shell_printf(shell, "OTP image version is not supported\n");
		return OTP_FAILURE;
	}
			...
			int bit_offset, int value, int nconfirm)
{
	...
	case OTP_REGION_STRAP:
		...
		ret = otp_strap_bit_confirm(shell, &strap_status[bit_offset], bit_offset, 0,
						value, 0);
		if (ret == OTP_FAILURE)
			return OTP_FAILURE;
...
static int otp_update_rid(const struct shell *shell, uint32_t update_num, int force)
{
	...
	ret = otp_get_sw_rid(dev, sw_rid);
	if (ret) {
		shell_printf(shell, "otp get sw rid failed\n");
		return ret;
	}
...
static int otp_retire_key(const struct shell *shell, uint32_t retire_id, int force)
{
	...
	if (ret) {
		shell_printf(shell, "otp get key num failed\n");
		return ret;
	}
...
static int do_otpread(const struct shell *shell, size_t argc, char **argv)
{
	...
	otp_dev_init(shell);
	ret = otp_begin_session(dev, &info_cb);
	if (ret) {
		shell_printf(shell, "otp begin session failed\n");
		return ret;
	}
...
static int do_otpprog(const struct shell *shell, size_t argc, char **argv)
{
	int addr;
	int ret;
	...
	if (argc == 3) {
		...
		ret = otp_prog_image(shell, addr, 1);

	} else if (argc == 2) {
		addr = strtoul(argv[1], NULL, 16);
		ret = otp_prog_image(shell, addr, 0);
	...
	if (mode == OTP_REGION_DATA) {
		...
		if (otp_addr != 4 && otp_addr != 10 && otp_addr != 11 && otp_addr < 16) {
			...
			if (*otp_rid64 > *sw_rid64) {
				shell_printf(shell, "update number could not bigger than "
						"current SW revision id\n");
				ret = -EINVAL;
			}
			...
			if (info_cb.pro_sts.pro_key_ret) {
				...
				if ((bit_offset >= 0 && bit_offset <= 7) ||
					...
					ret = otp_get_key_num(dev, &key_num);
					if (ret) {
						shell_printf(shell, "otp get key num failed\n");
						goto end;
					}
...
static int do_otpver(const struct shell *shell, size_t argc, char **argv)
{
	...
	ret = otp_get_tool_ver(dev, (char *)&otp_ver_str);
	if (ret) {
		shell_printf(shell, "otp get info failed\n");
		goto end;
	}
