...
static int otp_prog_data(struct otp_image_layout *image_layout, uint32_t *data)
{
	...
	for (i = 1792; i < 2046; i += 2) {
		...
		if (ret != OTP_SUCCESS) {
			LOG_ERR("address: %08x, data: %08x %08x, buffer: %08x %08x, "
				"mask: %08x %08x\n",
...
int aspeed_otp_prog_data(uint32_t offset, uint32_t *buf, uint32_t len)
{
	...
	for (int i = 0; i < len; i++) {
		...
		for (int j = 0; j < RETRY; j++) {
			if (verify_dw(prog_address, input_buf, ignore_mask,
				      compare, 1) != 0) {
				otp_soak(2);
				if (verify_dw(prog_address, input_buf, ignore_mask,
					      compare, 1) != 0) {
...
static int aspeed_otp_dt(const struct device *dev)
{
	...
	for (int i = 0; i < otpstrap_num * OTP_DT_OTPSTRAP_GROUP_NUM;
	     i += OTP_DT_OTPSTRAP_GROUP_NUM) {
