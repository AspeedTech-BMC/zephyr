...
static const struct ecdsa_testvec secp384r1[] = {
	...
	},
	{
		...
		.qy =
			...
			"\xFF\xE5\x38\x2D\x00\xCF\xCD\xA3\x03\x44\xA9\xB5\xB6\x8D\xB8\x55",
		.r =
			"\x1\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0"
			"\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0"
			"\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0",
		.s =
			"\x2\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0"
			"\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0"
			"\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0",
		.m =
			"\xF4\x92\xB9\xEB\x18\xA0\x6F\x7A\xA4\x79\x95\x3B\x31\xC3\x4F\xBF"
...
static int mbedtls_ecdsa_test(const struct shell *shell, int argc, char *argv[])
{
	...
	for (i = 0; i < ARRAY_SIZE(secp384r1); i++) {
		...
		if (ret) {
			...
			shell_fprintf(shell, SHELL_NORMAL, " Failed: %s\n", tmp);
#else
			shell_fprintf(shell, SHELL_NORMAL, " Failed: %d\n", ret);
#endif
		}

		if ((ret && !secp384r1[i].result) || (ret == 0 && secp384r1[i].result))
			shell_fprintf(shell, SHELL_NORMAL, " Test Pass\n");
		else
			shell_fprintf(shell, SHELL_NORMAL, " Test Failed\n");
...
static int hw_ecdsa_test(const struct shell *shell, int argc, char *argv[])
{
	...
	for (i = 0; i < ARRAY_SIZE(secp384r1); i++) {
		...
		shell_fprintf(shell, SHELL_NORMAL, " Test case %d...", i);
		ret = ecdsa_begin_session(dev, &ini, &ek);
		if (ret)
			shell_print(shell, "ecdsa_begin_session fail: %d", ret);

		ret = ecdsa_verify(&ini, &pkt);
		if (ret && !secp384r1[i].result)
			shell_fprintf(shell, SHELL_NORMAL, " result expected (failed), Pass\n");
		else if (ret == 0 && secp384r1[i].result)
			shell_fprintf(shell, SHELL_NORMAL, " result expected (pass), Pass\n");
		else
			shell_fprintf(shell, SHELL_NORMAL,
				      " result unexpected (ret=%d), Failed\n", ret);
