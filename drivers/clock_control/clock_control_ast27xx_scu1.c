...
{
	uint32_t rate = ast2700_soc1_get_pll_rate(scu, SCU1_CLK_HPLL);
...
{
	const struct clock_ast27xx_scu1_config *config = dev->config;
	struct ast2700_scu1 *scu = (struct ast2700_scu1 *)config->base;
	...
	case SCU1_CLK_APLL:
	case SCU1_CLK_DPLL:
		*rate = ast2700_soc1_get_pll_rate(scu, clk_id);
		break;
	case SCU1_CLK_AHB:
